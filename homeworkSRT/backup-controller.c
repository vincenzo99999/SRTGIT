...
void* back(void* par){
	...
	// La coda conterrà il messaggio di attivazione di controllo da parte del polling server
	mqd_t backup_queue;
	if ((backup_queue  = mq_open (BACKUP_QUEUE_NAME, O_RDONLY|O_CREAT, QUEUE_PERMISSIONS,&attr)) == -1) {
		perror ("Backup: mq_open (backup)");
		exit (1);
	}

    while (keep_on_running)
    {
        if(mq_receive(backup_queue, message,MAX_MSG_SIZE,NULL)==-1){
            //printf("ACTIVE 0");
            active=0;
            continue;
        }else{
    ...
    if (mq_close (backup_queue) == -1) {
        perror ("backup mq_close actuator_qd");
        exit (1);
    }
}
...
void * acquire_filter_loop(void * par) {
	...
	while (keep_on_running)
	{
        ...
        if(active==0){
			...
			shared_avg_sensor.value = reference;
...
void * control_loop(void * par) {
	...
	mqd_t reference2_qd;
	if ((reference2_qd = mq_open (REFERENCE2_QUEUE_NAME, O_RDONLY | O_CREAT | O_NONBLOCK, QUEUE_PERMISSIONS,&attr)) == -1) {
		perror ("control loop: mq_open (reference2)");
		exit (1);
	}
	...
	while (keep_on_running)
	{
		...
		if (mq_receive(reference2_qd, message,MAX_MSG_SIZE,NULL) == -1){
			//printf ("No reference ...\n");							//DEBUG
		}
		else{
...
void * diag (void * par){
	...
	mqd_t diag2_queue;
	if ((diag2_queue  = mq_open (DIAG2_QUEUE_NAME, O_RDONLY|O_CREAT, QUEUE_PERMISSIONS,&attr)) == -1) {
		perror ("backup-controll loop: mq_open (diag2)");
		exit (1);
	}

	while(keep_on_running){
		if (mq_receive(diag2_queue,message,MAX_MSG_SIZE,NULL) == -1){ 
...
int main(void)
{
    ...
    //BACK THREAD
    myparam.sched_priority = 52;
	pthread_attr_setschedparam(&myattr, &myparam); 
	pthread_create(&backup_thread,&myattr,back,NULL);

	//daig2 THREAD
	myparam.sched_priority = 44;
	pthread_attr_setschedparam(&myattr, &myparam); 
	pthread_create(&diag2_thread,&myattr,diag,NULL);
	...
	if (mq_unlink (REFERENCE2_QUEUE_NAME) == -1) {
        perror ("Main: mq_unlink reference2 queue");
        exit (1);
    }
	if (mq_unlink (BACKUP_QUEUE_NAME) == -1) {
        perror ("Main: mq_unlink backup queue");
        exit (1);
    }
	if (mq_unlink (PS_QUEUE_NAME) == -1) {
        perror ("Main: mq_unlink ps queue");
        exit (1);
    }	
